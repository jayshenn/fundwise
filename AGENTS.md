...
AKShare dictionary artifacts are maintained in `docs/AKShare数据字典.*` and regenerated from `docs/AKShare数据字典.html`.
...
- `uv run python scripts/export_akshare_dictionary.py`: export AKShare dictionary to CSV/JSON (with `full_url`) for retrieval workflows.
